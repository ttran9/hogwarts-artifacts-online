...
public class ArtifactControllerIntegrationTest {
    ...
    /*
     * @DirtiesContext resets the database before the below method is modified.
     * Although the testAddArtifactSuccess modifies the database the @DirtiesContext annotation will reset our h2 database.
     * The @DirtiesContext will cause our test to take longer because our Spring Boot app needs to be restart and the h2 database needs to be reset.
     */
    @Test
    @DirtiesContext(methodMode = DirtiesContext.MethodMode.BEFORE_METHOD)
    void testFindAllArtifactsSuccess() throws Exception {
        // we only mock the HTTP request.
        // we're testing the spring boot app from end to end. from the controller to the h2 database.
    ...
    void testUpdateArtifactErrorWithInvalidInput() throws Exception {
        ...
        this.mockMvc.perform(put(String.format("%s/artifacts/1250808601744904191", this.baseUrl))
        ...
        this.mockMvc.perform(get(String.format("%s/artifacts/1250808601744904191", this.baseUrl)).accept(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION, this.token))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS.getHttpStatusCodeValue()))
                .andExpect(jsonPath("$.message").value("Find One Success"))
                .andExpect(jsonPath("$.data.id").value("1250808601744904191"))
